...
import { limitesDeLImcGuide } from "./limites-de-l-imc";
...
describe("limites-de-l-imc — guide de référence limites", () => {
  const guide = limitesDeLImcGuide;

  it("est enregistré avec slug stable et charte IMC", () => {
    expect(getGuideBySlug("limites-de-l-imc")).toBeTruthy();
  ...
  it("organise FAQ, conclusion, en résumé, sources et note éditoriale", () => {
    ...
    expect(guide.introSummary?.items).toHaveLength(6);
  ...
  it("priorise les FAQ les plus recherchées", () => {
    expect(guide.faq[0]?.question).toBe("Pourquoi l'IMC est-il critiqué ?");
    expect(guide.faq[1]?.question).toBe("L'IMC est-il fiable ?");
    expect(guide.faq[2]?.question).toMatch(/sportifs/i);
    expect(guide.faq[3]?.question).toMatch(/médecins/i);
  });

  it("contient les sept limites majeures et des tableaux pédagogiques", () => {
    const tables = collectBlocks(guide).filter((block) => block.type === "table");
    expect(tables.length).toBeGreaterThanOrEqual(3);

    const limitsTable = tables.find(
      (block) => block.type === "table" && block.caption?.includes("Sept limites majeures"),
    );
    expect(limitsTable?.type === "table" && limitsTable.rows).toHaveLength(7);
  ...
  it("ne discrédite pas l'IMC et rappelle son utilité", () => {
    const content = JSON.stringify(guide);
    expect(content).not.toMatch(/l'IMC est inutile/i);
    expect(content).not.toMatch(/ne sert à rien/i);
    expect(content).toMatch(/repère/i);
    expect(content).toMatch(/OMS/i);
    expect(guide.conclusion.closingText).toContain("Comprendre les limites");
  });
